...
func main() {
	...
	q.Get("/v1/user", func(c *quick.Ctx) error {
		...
		return c.Status(200).String("Quick in action!") // Returns a success message
	})

	// Sets a GET route for "/v2"
	q.Get("/v2", func(c *quick.Ctx) error {
		c.Set("Content-Type", "application/json")     // Sets the content type as JSON
		return c.Status(200).String("Is in the air!") // Returns a message indicating that the service is active
	})

	// Sets a GET route for "/v3"
	q.Get("/v3", func(c *quick.Ctx) error {
		c.Set("Content-Type", "application/json") // Sets the content type as JSON
		return c.Status(200).String("Running!")   // Returns a message confirming that the server is running
	})
