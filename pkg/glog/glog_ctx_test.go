...
func TestNewCtx_DefaultKey(t *testing.T) {
	ctx, cancel := glog.NewCtx().Key("abc123").Build()
	defer cancel()
...
func TestNewCtx_CustomKeyName(t *testing.T) {
	ctx, cancel := glog.NewCtx().Name("X-User-ID").Key("user42").Build()
	defer cancel()
...
func TestNewCtx_Timeout(t *testing.T) {
	ctx, cancel := glog.NewCtx().Key("with-timeout").Timeout(10 * time.Millisecond).Build()
	defer cancel()
