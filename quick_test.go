...
// cover     -> go test -v -count=1 -cover -failfast -run ^TestQuick_PostListen$
// coverHTML -> go test -v -count=1 -failfast -cover -coverprofile=coverage.out -run ^TestQuick_PostListen$; go tool cover -html=coverage.out
func TestQuick_PostListen(t *testing.T) {
	type args struct {
		route       string
		wantCode    int
		wantOut     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			data, err := r.QuickTestListen(QuickMockTestServer{
